...
- **Un seul nœud** : pas de haute disponibilité. Si le VPS tombe, le site est indisponible.
...
- **CSP absente** : la Content Security Policy n'est pas configurée (identifié par ZAP, non corrigé en v1.0).
...
- **Ajouter un CSP header** dans la config nginx du container pour clôturer les warnings ZAP restants.
...
- **Multi-stage Dockerfile** : séparer la phase de build (si un compilateur est ajouté) de la phase de run pour réduire encore la taille de l'image.
