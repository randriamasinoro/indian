...
date: "2026-03-05"
tags: ["DevSecOps", "Docker", "Nginx", "CI/CD", "OWASP", "GitHub Actions"]
summary: "Un site vitrine de restaurant déployé en production avec une chaîne DevSecOps complète : analyse statique, scan de dépendances, tests dynamiques et déploiement automatisé sur Oracle Cloud ARM64."
domain: "devsecops"
---
...
C'est ici que le DAST a eu le plus de valeur concrète. La première exécution de ZAP a remonté une série de **WARN** sur des en-têtes de sécurité absents ou mal configurés. Chacun a été corrigé directement dans la configuration Nginx (`docker/nginx.conf`), puis re-vérifié au scan suivant. Le tableau ci-dessous résume chaque vulnérabilité, la correction appliquée, et **le risque concret si elle était passée inaperçue**.
...
> La leçon : un rapport DAST ne se lit pas comme une liste de bugs à corriger aveuglément. Chaque alerte demande une décision — corriger, ou justifier l'exclusion par écrit. C'est exactement la démarche d'analyse de risque attendue par le CRA.
...
- **Cloudflare en frontal** : proxy gratuit qui apporte DDoS protection, CDN et WAF basique — résoudrait le risque R5.
...
Ce projet a servi de terrain d'expérimentation pour traduire les exigences du CRA en livrables réels :
